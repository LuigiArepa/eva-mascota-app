...
- **Problema que resuelve**: Elimina la necesidad de encadenar múltiples llamadas `.then()` y `.catch()`, lo cual puede derivar en un código difícil de leer y mantener (conocido históricamente como _Callback Hell_ o _Promise Land_).
- **Ventajas**:
...
- **Funcionamiento**: `FormData` permite adjuntar archivos binarios reales directamente mediante `.append('imagen', imagenFile)`. Al enviar este objeto con Axios, el navegador establece automáticamente las cabeceras HTTP correctas como `Content-Type: multipart/form-data` y crea los límites (_boundaries_) necesarios para que el backend pueda descomponer y procesar el archivo correctamente.

### 4. ¿Qué son las props en React?

Las **props** (abreviatura de _properties_) son el mecanismo que utiliza React para pasar datos y configuraciones de un componente padre a un componente hijo.
...
- Pueden ser funciones de JavaScript que reciben _props_ y retornan elementos JSX (estructura visual de la interfaz).
